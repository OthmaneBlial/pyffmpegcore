...
WORKFLOW_CAPABILITY_RULES: dict[str, tuple[str, ...]] = {
    "convert": (),
    "compress": ("encoder:libx264", "encoder:aac"),
    "extract-audio": (),
    "thumbnail": ("filter:scale", "muxer:image2"),
    "waveform": ("filter:showwavespic", "muxer:image2"),
    "speed/video": ("filter:setpts", "filter:atempo", "encoder:libx264", "encoder:aac"),
    ...
    "mix-audio/concat": ("filter:acrossfade",),
    "mix-audio/mashup": ("filter:amix",),
...
class CapabilityInventory:
    ...
    def inspect(cls, binary: str = "ffmpeg") -> CapabilityInventory:
        ...
        return cls(
            ...
            muxers=_parse_table(_listing(binary, "-muxers"), 1),
            demuxers=_parse_table(_listing(binary, "-demuxers"), 1),
