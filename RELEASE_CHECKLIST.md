...
- Latest local artifact verification before the current documentation updates: two clean-archive builds at exact source SHA `d8019eeb355703effc95b7aafc44eb7adbfd3ba0` with `SOURCE_DATE_EPOCH=1790229624` produced identical wheel (`98,545` bytes; SHA-256 `5955db0e25a74fbb596ea781381c4d805e140198e1b931548ca656ab9db6d69f`) and sdist (`327,676` bytes; SHA-256 `de1c2c41534ac612129646235f6c21e6d7492795c5944a194fef3c76a9c09c24`). `twine check`, `check-wheel-contents`, and the sdist content contract passed. The exact wheel installed in a clean venv and the exact sdist in an isolated target with the local build backend; both passed `--version`, `doctor --json`, and `smoke-test --json` on macOS arm64, Python 3.14.6, FFmpeg 9.0.1. This is local evidence only. The progress and checklist updates change the sdist; rebuild from the final release SHA. No hosted OS matrix, attestation, tag, or publication exists for this build.
