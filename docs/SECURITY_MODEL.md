...
Commands reject an existing output unless the user passes `--force`. Receipts are created with owner-only permissions on POSIX systems; Windows uses filesystem ACLs. Batch and pipeline runners create fresh state files exclusively before execution, so a competing no-overwrite run fails instead of replacing the state. Resume and explicit `overwrite_state` deliberately allow state reuse. Directory image workflows reject symlinks at generated output filenames so an entry inside the selected output directory cannot redirect batch writes elsewhere. Explicit file paths remain caller-selected; PyFFmpegCore is not a filesystem sandbox. Temporary work should use private, randomly named directories and be removed after completion. Release and CI scripts must not place credentials or personal media into artifacts. The GitHub Action rejects existing symlinks in its workspace paths and rechecks artifact paths before upload. Symlink and time-of-check/time-of-use attacks are not fully prevented when another process can change a directory after validation; use a trusted working directory for hostile inputs.
