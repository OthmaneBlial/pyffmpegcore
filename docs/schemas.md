...
- `JobResult` uses `schema_version: "1.0"` and categorizes success, runtime failure, timeout, cancellation, and validation refusal. It includes the final normalized `ProgressEvent`, capture-policy output, warnings, output existence/size facts, and managed-workflow FFprobe verification state with format/stream evidence when available. Built-in profile contracts fail validation when a required output stream is missing or a present stream has a different expected codec or pixel format. `--preserve-all-streams` additionally compares input/output stream type, codec, and language, recording `stream_preservation` evidence or failing validation on a mismatch. FFprobe checks container metadata and streams; it does not decode the full output.
