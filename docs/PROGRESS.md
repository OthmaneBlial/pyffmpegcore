...
- Checkout: `main` at `25adc43150842930f84e25c105a7612f1a4b02f1`, synchronized with `origin/main` at audit start.
...
The audit found an inconsistency in the cross-platform text boundary. The managed executor and FFprobe parser decode tool output as UTF-8 with replacement, but the low-level runner, progress tracker, capability inspection, CLI doctor, and receipt version probe still use locale-dependent `text=True`. This contradicts the documented execution contract and can fail on non-UTF-8 locales. Next change: make FFmpeg/FFprobe text decoding consistent and add focused regression checks.
...
1. Fix locale-dependent decoding in every core FFmpeg/FFprobe subprocess path and add targeted tests.
